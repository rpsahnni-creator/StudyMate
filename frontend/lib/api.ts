...
export function getQuiz(quizId: string): Promise<QuizDetail> {
  return request<QuizDetail>(`${API_URL}/quizzes/${quizId}`);
}

export function startAttempt(quizId: string): Promise<StartAttemptResponse> {
  return request<StartAttemptResponse>(`${API_URL}/quizzes/${quizId}/attempts`, {
    method: "POST",
...
export function submitAttempt(
  ...
  return request<AttemptResult>(`${API_URL}/quizzes/${quizId}/attempts/${attemptId}/submit`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
...
export function getReview(quizId: string, attemptId: string): Promise<ReviewDetail> {
  return request<ReviewDetail>(`${API_URL}/quizzes/${quizId}/attempts/${attemptId}/review`);
}

export function getMyReports(page = 1, limit = 10): Promise<ReportsPage> {
  return request<ReportsPage>(`${API_URL}/users/me/reports?page=${page}&limit=${limit}`);
}
...
export function getAnalytics(): Promise<Analytics> {
  return request<Analytics>(`${API_URL}/users/me/analytics`);
}

export function getTopicAnalytics(): Promise<TopicAnalytics> {
  return request<TopicAnalytics>(`${API_URL}/users/me/analytics/topics`);
}
